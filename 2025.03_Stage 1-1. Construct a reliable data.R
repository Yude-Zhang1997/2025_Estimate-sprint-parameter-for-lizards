...
f_edge_top <- function(x, 
  ...
  x_top <- c(median(d[["edge_LT_x"]]), median(d[["edge_RT_x"]]))
  y_top <- c(median(d[["edge_LT_y"]]), median(d[["edge_RT_y"]]))
...
f_edge_bottom <- function(x, 
  ...
  x_bottom = c(median(d[["edge_LB_x"]]), median(d[["edge_RB_x"]]))
  y_bottom = c(median(d[["edge_LB_y"]]), median(d[["edge_RB_y"]]))
...
f_edge_left <- function(y, 
  ...
  x_left = c(median(d[["edge_LT_x"]]), median(d[["edge_LB_x"]]))
  y_left = c(median(d[["edge_LT_y"]]), median(d[["edge_LB_y"]]))
...
f_find_ID_in_edge <- function(d, 
  ...
  for (i in seq_along(d1_edge$ID)) {
    # Ensure values are not NA before comparison
    if (!is.na(d1_edge$Head_x[i]) &&
        !is.na(d1_edge$edge_left_x[i]) &&
        d1_edge$Head_x[i] > d1_edge$edge_left_x[i]) {
      if (!is.na(d1_edge$Head_y[i]) && !is.na(d1_edge$edge_top_y[i]) &&
          !is.na(d1_edge$edge_bottom_y[i]) &&
          d1_edge$Head_y[i] < d1_edge$edge_top_y[i] &&
          d1_edge$Head_y[i] > d1_edge$edge_bottom_y[i]) {
        ID_inedge <- append(ID_inedge, d1_edge$ID[i])
      }
    }
  }
  return(ID_inedge)
...
f_turn_rep_to_NA <-
  function(d_sprint_inedge, ID_replicate) {
    if (is.null(ID_replicate)) {
      ...
      d_sprint_removerep <- d_sprint_inedge %>%
        mutate(across(ends_with(c("_x", "_y")), ~ if_else(ID %in% (ID_replicate + 1), NA_real_, .)))
